<template><div><h1 id="进度事件和表单事件" tabindex="-1"><a class="header-anchor" href="#进度事件和表单事件" aria-hidden="true">#</a> 进度事件和表单事件</h1>
<h1 id="_1-文档的加载" tabindex="-1"><a class="header-anchor" href="#_1-文档的加载" aria-hidden="true">#</a> 1.文档的加载</h1>
<p>浏览器在加载一个页面时，是按照自上向下的顺序加载的，加载一行执行一行。
如果将js代码编写到页面的上边，当代码执行时，页面中的DOM对象还没有加载，
此时将会无法正常获取到DOM对象，导致DOM操作失败。</p>
<p>解决方式一：
可以将<code v-pre>js</code>代码编写到body的下边</p>
...
将<code v-pre>js</code>代码编写到<code v-pre>window.onload = function(){}</code>中
<code v-pre>window.onload </code>对应的回调函数会在整个页面加载完毕以后才执行，
所以可以确保代码执行时，DOM对象已经加载完毕了</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h1 id="_2-进度事件的种类" tabindex="-1"><a class="header-anchor" href="#_2-进度事件的种类" aria-hidden="true">#</a> 2.进度事件的种类</h1>
<p>进度事件用来描述资源加载的进度，主要由 AJAX 请求、<code v-pre>&lt;img&gt;</code>、<code v-pre>&lt;audio&gt;</code>、<code v-pre>&lt;video&gt;</code>、<code v-pre>&lt;style&gt;</code>、<code v-pre>&lt;link&gt;</code>等外部资源的加载触发，继承了<code v-pre>ProgressEvent</code>接口。它主要包含以下几种事件。</p>
<ul>
<li><code v-pre>abort</code>：外部资源中止加载时（比如用户取消）触发。如果发生错误导致中止，不会触发该事件。</li>
<li><code v-pre>error</code>：由于错误导致外部资源无法加载时触发。</li>
<li><code v-pre>load</code>：外部资源加载成功时触发。</li>
<li><code v-pre>loadstart</code>：外部资源开始加载时触发。</li>
<li><code v-pre>loadend</code>：外部资源停止加载时触发，发生顺序排在<code v-pre>error</code>、<code v-pre>abort</code>、<code v-pre>load</code>等事件的后面。</li>
<li><code v-pre>progress</code>：外部资源加载过程中不断触发。</li>
<li><code v-pre>timeout</code>：加载超时时触发。</li>
</ul>
...
<h1 id="_3-表单事件的种类" tabindex="-1"><a class="header-anchor" href="#_3-表单事件的种类" aria-hidden="true">#</a> 3.表单事件的种类</h1>
<h2 id="_3-1-input-事件" tabindex="-1"><a class="header-anchor" href="#_3-1-input-事件" aria-hidden="true">#</a> 3.1 input 事件</h2>
<p><code v-pre>input</code>事件当<code v-pre>&lt;input&gt;</code>、<code v-pre>&lt;select&gt;</code>、<code v-pre>&lt;textarea&gt;</code>的值发生变化时触发。对于复选框（<code v-pre>&lt;input type=checkbox&gt;</code>）或单选框（<code v-pre>&lt;input type=radio&gt;</code>），用户改变选项时，也会触发这个事件。另外，对于打开<code v-pre>contenteditable</code>属性的元素，只要值发生变化，也会触发<code v-pre>input</code>事件。</p>
<p><code v-pre>input</code>事件的一个特点，就是会连续触发，比如用户每按下一次按键，就会触发一次<code v-pre>input</code>事件。</p>
<p><code v-pre>input</code>事件对象继承了<code v-pre>InputEvent</code>接口。</p>
<p>该事件跟<code v-pre>change</code>事件很像，不同之处在于<code v-pre>input</code>事件在元素的值发生变化后立即发生，而<code v-pre>change</code>在元素失去焦点时发生，而内容此时可能已经变化多次。也就是说，如果有连续变化，<code v-pre>input</code>事件会触发多次，而<code v-pre>change</code>事件只在失去焦点时触发一次。</p>
...
<h2 id="_3-2-select-事件" tabindex="-1"><a class="header-anchor" href="#_3-2-select-事件" aria-hidden="true">#</a> 3.2 select 事件</h2>
<p><code v-pre>select</code>事件当在<code v-pre>&lt;input&gt;</code>、<code v-pre>&lt;textarea&gt;</code>里面选中文本时触发。</p>
...
<h2 id="_3-3-change-事件" tabindex="-1"><a class="header-anchor" href="#_3-3-change-事件" aria-hidden="true">#</a> 3.3 change 事件</h2>
<p><code v-pre>change</code>事件当<code v-pre>&lt;input&gt;</code>、<code v-pre>&lt;select&gt;</code>、<code v-pre>&lt;textarea&gt;</code>的值发生变化时触发。它与<code v-pre>input</code>事件的最大不同，就是不会连续触发，只有当全部修改完成时才会触发，另一方面<code v-pre>input</code>事件必然伴随<code v-pre>change</code>事件。具体来说，分成以下几种情况。</p>
<ul>
...
<h2 id="_3-4-invalid-事件" tabindex="-1"><a class="header-anchor" href="#_3-4-invalid-事件" aria-hidden="true">#</a> 3.4 invalid 事件</h2>
<p>用户提交表单时，如果表单元素的值不满足校验条件，就会触发<code v-pre>invalid</code>事件。</p>
...
<h2 id="_3-5-reset-事件-submit-事件" tabindex="-1"><a class="header-anchor" href="#_3-5-reset-事件-submit-事件" aria-hidden="true">#</a> 3.5 reset 事件，submit 事件</h2>
<p>这两个事件发生在表单对象<code v-pre>&lt;form&gt;</code>上，而不是发生在表单的成员上。</p>
<p><code v-pre>reset</code>事件当表单重置（所有表单成员变回默认值）时触发。</p>
<p><code v-pre>submit</code>事件当表单数据向服务器提交时触发。注意，<code v-pre>submit</code>事件的发生对象是<code v-pre>&lt;form&gt;</code>元素，而不是<code v-pre>&lt;button&gt;</code>元素，因为提交的是表单，而不是按钮。</p>
</div></template>
