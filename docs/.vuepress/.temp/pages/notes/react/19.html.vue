<template><div><h1 id="_19-【rtk-query】" tabindex="-1"><a class="header-anchor" href="#_19-【rtk-query】" aria-hidden="true">#</a> 19 【RTK Query】</h1>
<h2 id="_1-目前前端常见的发起-ajax-请求的方式" tabindex="-1"><a class="header-anchor" href="#_1-目前前端常见的发起-ajax-请求的方式" aria-hidden="true">#</a> 1.目前前端常见的发起 ajax 请求的方式</h2>
<ul>
<li>1、使用原生的<code v-pre>ajax</code>请求</li>
<li>2、使用<code v-pre>jquery</code>封装好的<code v-pre>ajax</code>请求</li>
<li>3、使用<code v-pre>fetch</code>发起请求</li>
<li>4、第三方的比如<code v-pre>axios</code>请求</li>
<li>5、<code v-pre>angular</code>中自带的<code v-pre>HttpClient</code></li>
</ul>
<p>就目前前端框架开发中来说我们在开发<code v-pre>vue</code>、<code v-pre>react</code>的时候一般都是使用<code v-pre>fetch</code>或<code v-pre>axios</code>自己封装一层来与后端数据交互，至于<code v-pre>angular</code>肯定是用自带的<code v-pre>HttpClient</code>请求方式，但是依然存在几个致命的弱点，</p>
<ul>
<li>1、对当前请求数据不能缓存，</li>
<li>2、一个页面上由多个组件组成，但是刚好有遇到复用相同组件的时候，那么就会发起多次<code v-pre>ajax</code>请求</li>
</ul>
<blockquote>
<p>📢 针对同一个接口发起多次请求的解决方法，目前常见的解决方案</p>
</blockquote>
<ul>
<li>1、使用<code v-pre>axios</code>的取消发起请求，<a href="http://www.axios-js.com/zh-cn/docs/#%E5%8F%96%E6%B6%88" target="_blank" rel="noopener noreferrer">参考文档<ExternalLinkIcon/></a></li>
<li>2、<code v-pre>vue</code>中还没看到比较好的方法</li>
<li>3、在<code v-pre>rect</code>中可以借用类似<a href="https://react-query.tanstack.com/" target="_blank" rel="noopener noreferrer">react-query<ExternalLinkIcon/></a>工具对请求包装一层</li>
<li>4、对于<code v-pre>angular</code>中直接使用<code v-pre>rxjs</code>的操作符<code v-pre>shareReplay</code></li>
</ul>
<h2 id="_2-rtk-query-概述" tabindex="-1"><a class="header-anchor" href="#_2-rtk-query-概述" aria-hidden="true">#</a> 2.RTK Query 概述</h2>
...
<p><code v-pre>rtk-query</code>是<a href="https://redux-toolkit.js.org/" target="_blank" rel="noopener noreferrer"><code v-pre>redux-toolkit</code><ExternalLinkIcon/></a>里面的一个分之，专门用来优化前端接口请求，目前也只支持在<code v-pre>react</code>中使用。</p>
<p><strong>RTK Query</strong> 是一个强大的数据获取和缓存工具。它旨在简化在 Web 应用程序中加载数据的常见情况，<strong>无需自己手动编写数据获取和缓存逻辑</strong>。</p>
<p>RTK Query 是<strong>一个包含在 Redux Toolkit 包中的可选插件</strong>，其功能构建在 Redux Toolkit 中的其他 API 之上。</p>
<p><strong>Web应用中加载数据时需要处理的问题：</strong></p>
<ol>
<li>根据不同的加载状态显示不同UI组件</li>
<li>减少对相同数据重复发送请求</li>
<li>使用乐观更新，提升用户体验</li>
<li>在用户与UI交互时，管理缓存的生命周期</li>
</ol>
...
<p>我们将 <code v-pre>createAsyncThunk</code> 与 <code v-pre>createSlice</code> 一起使用，在发出请求和管理加载状态方面仍然需要进行大量手动工作。我们必须创建异步 thunk，发出实际请求，从响应中提取相关字段，添加加载状态字段，在 <code v-pre>extraReducers</code> 中添加处理程序以处理 <code v-pre>pending/fulfilled/rejected</code> 情况，并实际编写正确的状态更新。</p>
<p>在过去的几年里，React 社区已经意识到 <strong>“数据获取和缓存” 实际上是一组不同于 “状态管理” 的关注点</strong>。虽然你可以使用 Redux 之类的状态管理库来缓存数据，但用例差异较大，因此值得使用专门为数据获取用例构建的工具。</p>
<p>RTK Query 在其 API 设计中添加了独特的方法：</p>
<ul>
<li>数据获取和缓存逻辑构建在 Redux Toolkit 的 <code v-pre>createSlice</code> 和 <code v-pre>createAsyncThunk</code> API 之上</li>
<li>由于 Redux Toolkit 与 UI 无关，因此 RTK Query 的功能可以与任何 UI 层一起使用</li>
<li>API 请求接口是提前定义的，包括如何从参数生成查询参数和转换响应以进行缓存</li>
<li>RTK Query 还可以生成封装整个数据获取过程的 React hooks ，为组件提供 <code v-pre>data</code> 和 <code v-pre>isFetching</code> 字段，并在组件挂载和卸载时管理缓存数据的生命周期</li>
...
<blockquote>
<p>📢 <code v-pre>rtk-query</code>的使用环境，必须是<code v-pre>react</code>版本大于 17,可以使用<code v-pre>hooks</code>的版本，因为使用<code v-pre>rtk-query</code>的查询都是<code v-pre>hooks</code>的方式，如果你项目简单<code v-pre>redux</code>都未使用到，本人不建议你用<code v-pre>rtk-query</code>，可能直接使用<code v-pre>axios</code>请求更加的简单方便。</p>
</blockquote>
<h2 id="_3-基础开发流程" tabindex="-1"><a class="header-anchor" href="#_3-基础开发流程" aria-hidden="true">#</a> 3.基础开发流程</h2>
<blockquote>
<p>后面这些案例后端接口返回格式都是</p>
...
<li>创建一个festures/api文件夹用来装所有的API Slice</li>
<li>创建一个sudentApiSlice.js文件，并导出简单的加减方法</li>
</ul>
<h3 id="_3-1-定义-api-slice" tabindex="-1"><a class="header-anchor" href="#_3-1-定义-api-slice" aria-hidden="true">#</a> 3.1 定义 API Slice</h3>
<p>使用 RTK Query，<strong>管理缓存数据的逻辑被集中到每个应用程序的单个“API Slice”中</strong>。就像每个应用程序只有一个 Redux 存储一样，我们现在有一个Slice 用于 <em>所有</em> 我们的缓存数据。</p>
<p>我们将从定义一个新的 <code v-pre>sudentApiSlice.js</code> 文件开始。由于这不是特定于我们已经编写的任何其他“功能”，我们将添加一个新的 <code v-pre>features/api/</code> 文件夹并将 <code v-pre>sudentApiSlice.js</code> 放在那里。让我们填写 API Slice 文件，然后分解里面的代码，看看它在做什么：</p>
<p><code v-pre>features/api/sudentApiSlice.js</code></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 从特定于 React 的入口点导入 RTK Query 方法</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上例是一个比较简单的Api对象的例子，我们来分析一下，首先我们需要调用<code v-pre>createApi()</code>来创建Api对象。这个方法在RTK中存在两个版本，一个位于<code v-pre>@reduxjs/toolkit/dist/query</code>下，一个位于<code v-pre>@reduxjs/toolkit/dist/query/react</code>下。react目录下的版本会自动生成一个钩子，方便我们使用Api。如果不要钩子，可以引入query下的版本，当然我不建议你这么做。</p>
<p><code v-pre>createApi()</code>需要一个配置对象作为参数，配置对象中的属性繁多，我们暂时介绍案例中用到的属性：</p>
<p><strong>reducerPath</strong></p>
<p>用来设置reducer的唯一标识，主要用来在创建store时指定action的type属性，如果不指定默认为api。</p>
<p><strong>baseQuery</strong></p>
<p>用来设置发送请求的工具，就是你是用什么发请求，RTKQ为我们提供了fetchBaseQuery作为查询工具，它对fetch进行了简单的封装，很方便，如果你不喜欢可以改用其他工具，这里暂时不做讨论。</p>
<p><strong>fetchBaseQuery</strong></p>
...
<p>查询：<code v-pre>build.query({})</code></p>
<p>增删改：<code v-pre>build.mutation({})</code></p>
<p>例如：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token literal-property property">getStudents</span><span class="token operator">:</span> builder<span class="token punctuation">.</span><span class="token function">query</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  <span class="token comment">// 请求的 URL 是“/api/all/student”</span>
  <span class="token function-variable function">query</span><span class="token operator">:</span> <span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token string">'/all/student'</span><span class="token punctuation">,</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">,</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>先说query，query也需要一个配置对象作为参数。配置对象里同样有n多个属性，现在直说一个，query方法。注意不要搞混两个query，一个是build的query方法，一个是query方法配置对象中的属性，这个方法需要返回一个子路径，这个子路径将会和baseUrl拼接为一个完整的请求路径。比如：getStudets的最终请求地址是:</p>
<div class="language-absh line-numbers-mode" data-ext="absh"><pre v-pre class="language-absh"><code>http://localhost:8080/api + /all/student = http://localhost:8080/api/all/student
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>可算是介绍完了，但是注意了这个只是最基本的配置。RTKQ功能非常强大，但是配置也比较麻烦。不过，熟了就好了。</p>
...
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">export</span> <span class="token keyword">const</span> <span class="token punctuation">{</span>useGetStudentsQuery<span class="token punctuation">}</span> <span class="token operator">=</span> studentApi<span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="_3-2-配置-store" tabindex="-1"><a class="header-anchor" href="#_3-2-配置-store" aria-hidden="true">#</a> 3.2 配置 Store</h3>
<p>我们现在需要将 API Slice 连接到我们的 Redux 存储。我们可以修改现有的 <code v-pre>store.js</code> 文件，将 API slice 的 cache reducer 添加到状态中。此外，API slice 会生成需要添加到 store 的自定义 middleware。这个 middleware <em>必须</em> 被添加——它管理缓存的生命周期和控制是否过期。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们可以在 <code v-pre>reducer</code> 参数中重用 <code v-pre>sudentApiSlice.reducerPath</code> 字段作为计算键，以确保在正确的位置添加缓存 reducer。</p>
<p>我们需要在 store 设置中保留所有现有的标准 middleware，例如“redux-thunk”，而 API slice 的 middleware 通常会在这些 middleware 之后使用。我们可以通过向 <code v-pre>configureStore</code> 提供 <code v-pre>middleware</code> 参数，调用提供的 <code v-pre>getDefaultMiddleware()</code> 方法，并在返回的 middleware 数组的末尾添加 <code v-pre>sudentApiSlice.middleware</code> 来做到这一点。</p>
<p>store创建完毕同样要设置Provider标签，这里不再展示。</p>
<h3 id="_3-3-在组件中使用-query-hooks" tabindex="-1"><a class="header-anchor" href="#_3-3-在组件中使用-query-hooks" aria-hidden="true">#</a> 3.3 在组件中使用 Query Hooks</h3>
<p>接下来，我们来看看如果通过studentApi发送请求。由于我们已经将studentApi中的钩子函数向外部导出了，所以我们只需通过钩子函数即可自动加载到所有的学生信息。比如，现在在App.js中加载信息可以这样编写代码：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们能够用对 <code v-pre>useGetStudentsQuery()</code> 的单个调用来替换多个 <code v-pre>useSelector</code> 调用和 <code v-pre>useEffect</code> 调度。</p>
<p>直接调用<code v-pre>useGetStudentsQuery()</code>它会自动向服务器发送请求加载数据，每个生成的 Query hooks 都会返回一个包含多个字段的“结果”对象，包括：</p>
<ol>
<li><code v-pre>data</code> – 来自服务器的实际响应内容。 <strong>在收到响应之前，该字段将是 “undefined”</strong>。</li>
<li><code v-pre>currentData</code> – 当前的数据</li>
<li><code v-pre>isUninitialized</code> – 如果为true则表示查询还没开始</li>
<li><code v-pre>data</code>:来自服务器的实际响应内容。 <strong>在收到响应之前，该字段将是 “undefined”</strong>。</li>
<li><code v-pre>isLoading</code>: 一个 boolean，指示此 hooks 当前是否正在向服务器发出 <em>第一次</em> 请求。（请注意，如果参数更改以请求不同的数据，<code v-pre>isLoading</code> 将保持为 false。）</li>
<li><code v-pre>isFetching</code>: 一个 boolean，指示 hooks 当前是否正在向服务器发出 <em>any</em> 请求</li>
<li><code v-pre>isSuccess</code>: 一个 boolean，指示 hooks 是否已成功请求并有可用的缓存数据（即，现在应该定义 data）</li>
<li><code v-pre>isError</code>: 一个 boolean，指示最后一个请求是否有错误</li>
<li><code v-pre>error</code>: 一个 serialized 错误对象</li>
<li><code v-pre>refetch </code>函数，用来重新加载数据</li>
</ol>
<p>从结果对象中解构字段是很常见的，并且可能将 <code v-pre>data</code> 重命名为更具体的变量，例如 <code v-pre>studentRes</code> 来描述它包含的内容。然后我们可以使用状态 boolean 和 <code v-pre>data/error</code> 字段来呈现我们想要的 UI。 但是，如果你使用的是 TypeScript，你可能需要保持原始对象不变，并在条件检查中将标志引用为 <code v-pre>result.isSuccess</code>，以便 TS 可以正确推断 <code v-pre>data</code> 是有效的。</p>
<p><img src="https://i0.hdslb.com/bfs/album/f1cc5755cb70b40751511b3dbf6ebdb2cfab7133.png" alt="Snipaste_2022-11-04_22-57-20"></p>
<p>这是最终效果：</p>
<p><img src="https://i0.hdslb.com/bfs/album/e0818fbcb9a2c521bcf4ded7d67336911813198e.png" alt="image-20221104231936997"></p>
<h2 id="_4-传递参数" tabindex="-1"><a class="header-anchor" href="#_4-传递参数" aria-hidden="true">#</a> 4.传递参数</h2>
<h3 id="_4-1-定义接收参数" tabindex="-1"><a class="header-anchor" href="#_4-1-定义接收参数" aria-hidden="true">#</a> 4.1 定义接收参数</h3>
<p><code v-pre>features/api/sudentApiSlice.js</code></p>
<p>这里定义了一个新的接口，通过id获取学生信息</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 从特定于 React 的入口点导入 RTK Query 方法</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> createApi<span class="token punctuation">,</span> fetchBaseQuery <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'@reduxjs/toolkit/dist/query/react'</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_4-2-传递参数" tabindex="-1"><a class="header-anchor" href="#_4-2-传递参数" aria-hidden="true">#</a> 4.2 传递参数</h3>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token keyword">function</span> <span class="token function">App</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token keyword">return</span> <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>div</span><span class="token punctuation">></span></span><span class="token punctuation">{</span>content<span class="token punctuation">}</span><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>div</span><span class="token punctuation">></span></span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>useGetPostQuery</code>这个钩子接收的第一个参数传递到<code v-pre>query</code>方法，使用起来很简单</p>
<p><img src="https://i0.hdslb.com/bfs/album/8f9e3ec4a807396819fcb6d9ef6628816da0df14.png" alt="image-20221105135329712"></p>
<h2 id="_5-转换响应" tabindex="-1"><a class="header-anchor" href="#_5-转换响应" aria-hidden="true">#</a> 5.转换响应</h2>
<p><strong>请求接口可以定义一个 <code v-pre>transformResponse</code> 处理程序，该处理程序可以在缓存之前提取或修改从服务器接收到的数据</strong>。我们可以有 <code v-pre>transformResponse: (responseData) =&gt; responseData.data</code>，它只会缓存实际的 <code v-pre>student</code> 对象，而不是整个响应体。</p>
<p><code v-pre>features/api/sudentApiSlice.js</code></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token literal-property property">getStudentById</span><span class="token operator">:</span> builder<span class="token punctuation">.</span><span class="token function">query</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  <span class="token function-variable function">query</span><span class="token operator">:</span> <span class="token parameter">sutId</span> <span class="token operator">=></span> <span class="token template-string"><span class="token template-punctuation string">`</span><span class="token string">/student/</span><span class="token interpolation"><span class="token interpolation-punctuation punctuation">${</span>sutId<span class="token interpolation-punctuation punctuation">}</span></span><span class="token template-punctuation string">`</span></span><span class="token punctuation">,</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">,</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>对于上一个案例中通过id获取学生信息的接口，加一个<code v-pre>transformResponse</code>方法，我们来看看他接受到的参数<code v-pre>responseData</code>是什么</p>
<p><img src="https://i0.hdslb.com/bfs/album/8e08b7d67470ca1eeafadf751a723966090f855e.png" alt="image-20221105140454626"></p>
<p>可以看到<code v-pre>responseData</code>这个参数就是我们的响应体</p>
<p>在使用的过程中，发现这个方法类似于响应拦截器。</p>
<p>我们在<code v-pre>App.jsx</code>中看看<code v-pre>useGetStudentByIdQuery</code>这个钩子函数返回的<code v-pre>data</code>有什么变化</p>
<p><img src="https://i0.hdslb.com/bfs/album/79bbe6858e5a55b3f62201d3db8643984d69e29b.png" alt="image-20221105140911763"></p>
<h2 id="_6-rtk-query-缓存简单介绍" tabindex="-1"><a class="header-anchor" href="#_6-rtk-query-缓存简单介绍" aria-hidden="true">#</a> 6.RTK Query 缓存简单介绍</h2>
<blockquote>
<p>后面在介绍缓存的灵活使用</p>
</blockquote>
<h3 id="_6-1-什么是相同查询" tabindex="-1"><a class="header-anchor" href="#_6-1-什么是相同查询" aria-hidden="true">#</a> 6.1 什么是相同查询</h3>
<p>RTK Query 会将查询查询参数序列化为字符串，并将相同钩子、相同参数的查询视为相同的查询，他们将共享一个请求与缓存数据。</p>
<p>因此，下面两个调用返回结果相同（即使在不同的组件中）：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token function">useGetXXXQuery</span><span class="token punctuation">(</span><span class="token punctuation">{</span> <span class="token literal-property property">a</span><span class="token operator">:</span> <span class="token number">1</span><span class="token punctuation">,</span> <span class="token literal-property property">b</span><span class="token operator">:</span> <span class="token number">2</span> <span class="token punctuation">}</span><span class="token punctuation">)</span> <span class="token comment">// 订阅 + 1</span>
...
<ul>
<li>他们使用相同的查询：GetXXX</li>
<li>查询参数的序列化结果相同：<code v-pre>'{&quot;a&quot;:1,&quot;b&quot;:2}'</code></li>
</ul>
<p>你不需要担心嵌套或是字段顺序，或是不同对象不同引用会被认为是不同的查询，因为 RTK Query 已经在默认的序列化函数中处理了相关用例。同时，你也可以提供自己的序列化函数。</p>
<h3 id="_6-2-引用计数与垃圾回收" tabindex="-1"><a class="header-anchor" href="#_6-2-引用计数与垃圾回收" aria-hidden="true">#</a> 6.2 引用计数与垃圾回收</h3>
<p>当在组件中使用某个查询时，该查询的引用计数会 + 1，当该组件被卸载时，引用计数会 -1。当一个查询的引用计数为 0 时，说明没有任何组件在使用这个查询。此时，经过 <code v-pre>keepUnusedDataFor</code>（默认为 30 ）秒后，如果缓存仍为被使用过，那么他将被从缓存中移除。</p>
<h3 id="_6-3-缓存初体验" tabindex="-1"><a class="header-anchor" href="#_6-3-缓存初体验" aria-hidden="true">#</a> 6.3 缓存初体验</h3>
<p>缓存的配置</p>
<p><code v-pre>store/index.js</code></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// configureStore创建一个redux数据</span>
<span class="token keyword">const</span> store <span class="token operator">=</span> <span class="token function">configureStore</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  <span class="token comment">// 合并多个Slice</span>
  <span class="token literal-property property">reducer</span><span class="token operator">:</span> <span class="token punctuation">{</span>
    <span class="token punctuation">[</span>sudentApiSlice<span class="token punctuation">.</span>reducerPath<span class="token punctuation">]</span><span class="token operator">:</span> sudentApiSlice<span class="token punctuation">.</span>reducer<span class="token punctuation">,</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>其实就是在这个<code v-pre>store</code>配置这个中间件，一开始就配好了。</p>
<p>来看看实际效果</p>
<p>先改写下上面的案例</p>
<p><code v-pre>App.jsx</code></p>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token keyword">function</span> <span class="token function">App</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">)</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>StudentA.jsx</code></p>
<div class="language-jsx line-numbers-mode" data-ext="jsx"><pre v-pre class="language-jsx"><code><span class="token keyword">import</span> React <span class="token keyword">from</span> <span class="token string">'react'</span>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token keyword">function</span> <span class="token function">Student</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">)</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>StudentB.jsx</code></p>
<div class="language-jsx line-numbers-mode" data-ext="jsx"><pre v-pre class="language-jsx"><code><span class="token keyword">import</span> React <span class="token keyword">from</span> <span class="token string">'react'</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> useEffect <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'react'</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> useGetStudentByIdQuery <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'./store/features/api/sudentApiSlice'</span>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token keyword">function</span> <span class="token function">Student</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">)</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们把学生信息抽离成两个组件，里面除了标题都是一样的，在<code v-pre>App.jsx</code>中设置了个三个按钮控制显示隐藏</p>
<p>切换到<code v-pre>StudentA</code>组件</p>
<p><img src="https://i0.hdslb.com/bfs/album/82cdd432c4ba92162d0b00a97c17086e4319de99.png" alt="image-20221105151052394"></p>
<p>切换到<code v-pre>StudentB</code>组件</p>
<p><img src="https://i0.hdslb.com/bfs/album/5cb7363a0c93dbea2ce3c1a6f2243ba4012e57c4.png" alt="image-20221105151112395"></p>
<p>可以看到切换到<code v-pre>StudentB</code>组件并没有重新发起请求，这就是缓存生效了。</p>
<p><strong>RTK Query 允许多个组件订阅相同的数据，并且将确保每个唯一的数据集只获取一次。</strong> 在内部，RTK Query 为每个请求接口 + 缓存键组合保留一个 action 订阅的引用计数器。如果组件 A 调用 <code v-pre>useGetStudentByIdQuery(stuId)</code>，则将获取该数据。如果组件 B 挂载并调用 <code v-pre>useGetStudentByIdQuery(stuId)</code>，则请求的数据完全相同。两种钩子用法将返回完全相同的结果，包括获取的 “data” 和加载状态标志。</p>
<p>当活跃订阅数下降到 0 时，RTK Query 会启动一个内部计时器。 <strong>如果在添加任何新的数据订阅之前计时器到期，RTK Query 将自动从缓存中删除该数据</strong>，因为应用程序不再需要该数据。但是，如果在计时器到期之前添加了新订阅，则取消计时器，并使用已缓存的数据而无需重新获取它。</p>
<p>在这种情况下，我们的 <code v-pre>&lt;StudentA&gt;</code> 挂载并通过 ID 请求。当我们“切换” 时，<code v-pre>&lt;StudentA&gt;</code> 组件被路由器卸载，并且活动订阅由于卸载而被删除。RTK Query 立即启动 “remove this post data” 计时器。但是，<code v-pre>&lt;StudentB&gt;</code> 组件会立即挂载并使用相同的缓存键订阅相同的 <code v-pre>student</code> 数据。因此，RTK Query 取消了计时器并继续使用相同的缓存数据，而不是从服务器获取数据。</p>
<p>默认情况下，<strong>未使用的数据会在 60 秒后从缓存中删除</strong>，但这可以在根 API Slice 定义中进行配置，也可以使用 <code v-pre>keepUnusedDataFor</code> 标志在各个请求接口定义中覆盖，该标志指定缓存生存期 秒。</p>
<p><code v-pre>features/api/sudentApiSlice.js</code></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token literal-property property">getStudentById</span><span class="token operator">:</span> builder<span class="token punctuation">.</span><span class="token function">query</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  <span class="token comment">// 从query方法这里接收参数</span>
  <span class="token function-variable function">query</span><span class="token operator">:</span> <span class="token parameter">sutId</span> <span class="token operator">=></span> <span class="token template-string"><span class="token template-punctuation string">`</span><span class="token string">/student/</span><span class="token interpolation"><span class="token interpolation-punctuation punctuation">${</span>sutId<span class="token interpolation-punctuation punctuation">}</span></span><span class="token template-punctuation string">`</span></span><span class="token punctuation">,</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">,</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_7-mutation-请求接口" tabindex="-1"><a class="header-anchor" href="#_7-mutation-请求接口" aria-hidden="true">#</a> 7.mutation 请求接口</h2>
<p>我们已经看到了如何通过定义查询请求接口从服务器获取数据，但是向服务器发送更新呢？</p>
<p>RTK Query 让我们定义 <strong>mutation 请求接口</strong> 来更新服务器上的数据。让我们添加一个可以让我们添加新学生的 Mutation。</p>
<h3 id="_7-1-添加新的-mutations-后请求接口" tabindex="-1"><a class="header-anchor" href="#_7-1-添加新的-mutations-后请求接口" aria-hidden="true">#</a> 7.1 添加新的 Mutations 后请求接口</h3>
<p>添加 Mutation 请求接口与添加查询请求接口非常相似。 最大的不同是我们使用 <code v-pre>builder.mutation()</code> 而不是 <code v-pre>builder.query()</code> 来定义请求接口。 此外，我们现在需要将 HTTP 方法更改为“POST”请求，并且我们还必须提供请求的正文。</p>
<p><code v-pre>features/api/sudentApiSlice.js</code></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> createApi<span class="token punctuation">,</span> fetchBaseQuery <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'@reduxjs/toolkit/dist/query/react'</span>
...
<span class="token keyword">export</span> <span class="token keyword">const</span> <span class="token punctuation">{</span> useGetStudentsQuery<span class="token punctuation">,</span> useGetStudentByIdQuery<span class="token punctuation">,</span> useAddNewStudentMutation <span class="token punctuation">}</span> <span class="token operator">=</span>
  sudentApiSlice
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里我们的 <code v-pre>query</code> 选项返回一个包含 <code v-pre>{url, method, body}</code> 的对象。 由于我们使用 <code v-pre>fetchBaseQuery</code> 来发出请求，<code v-pre>body</code> 字段将自动为我们进行 JSON 序列化。</p>
<p>与查询请求接口一样，API slice 会自动为 Mutation 请求接口生成一个 React hooks - 在本例中为 <code v-pre>useAddNewPostMutation</code>。</p>
<h3 id="_7-2-在组件中使用-mutation-hooks" tabindex="-1"><a class="header-anchor" href="#_7-2-在组件中使用-mutation-hooks" aria-hidden="true">#</a> 7.2 在组件中使用 Mutation Hooks</h3>
<p>每当我们单击“添加”按钮时，我们以前得调度了一个异步 thunk 来添加。 为此，它必须导入 <code v-pre>useDispatch</code> 和 <code v-pre>addNewPost</code> thunk。 Mutation hooks 取代了这两者，并且使用模式非常相似。</p>
<div class="language-jsx line-numbers-mode" data-ext="jsx"><pre v-pre class="language-jsx"><code><span class="token keyword">import</span> React<span class="token punctuation">,</span> <span class="token punctuation">{</span> useState <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'react'</span>
...
<ul>
<li>第一个值是触发函数。当被调用时，它会使用你提供的任何参数向服务器发出请求。这实际上就像一个已经被包装以立即调度自身的 thunk。</li>
<li>第二个值是一个对象，其中包含有关当前正在进行的请求（如果有）的元数据。这包括一个 <code v-pre>isLoading</code> 标志以指示请求是否正在进行中。</li>
</ul>
<p>我们可以用 <code v-pre>useAddNewStudentMutation</code> hooks 中的触发函数和 <code v-pre>isLoading</code> 标志替换现有的 thunk 调度和组件加载状态，组件的其余部分保持不变。</p>
<p>与 thunk 调度一样，我们使用初始 post 对象调用 <code v-pre>addNewStudent</code>。 这会返回一个带有<code v-pre>.unwrap()</code>方法的特殊 Promise ，我们可以 <code v-pre>await addNewStudent().unwrap()</code> 使用标准的 <code v-pre>try/catch</code> 块来处理任何潜在的错误。</p>
<p><img src="https://i0.hdslb.com/bfs/album/218da1adc96ca99b2f9ee57c5b7a8c96ef48448d.png" alt="image-20221105193209910"></p>
<h2 id="_8-usequery-hook-参数" tabindex="-1"><a class="header-anchor" href="#_8-usequery-hook-参数" aria-hidden="true">#</a> 8.useQuery Hook 参数</h2>
<p>查询钩子需要两个参数：<code v-pre>(queryArg?, queryOptions?)</code></p>
<p>参数将被传递到底层回调以生成URL。<code v-pre>queryArg</code> <code v-pre>query</code></p>
<p>该对象接受几个可用于控制数据获取行为的附加参数：<code v-pre>queryOptions</code></p>
<ul>
<li><a href="https://redux-toolkit.js.org/rtk-query/usage/conditional-fetching" target="_blank" rel="noopener noreferrer">skip<ExternalLinkIcon/></a>  - 允许查询“跳过”为该渲染运行。默认为<code v-pre>false</code></li>
<li><a href="https://redux-toolkit.js.org/rtk-query/usage/polling" target="_blank" rel="noopener noreferrer">pollingInterval<ExternalLinkIcon/></a>  - 允许查询按提供的时间间隔（以毫秒为单位指定）自动重新获取。默认为*（关闭）*<code v-pre>0</code></li>
<li><a href="https://redux-toolkit.js.org/rtk-query/usage/queries#selecting-data-from-a-query-result" target="_blank" rel="noopener noreferrer">selectFromResult<ExternalLinkIcon/></a> - 允许更改钩子的返回值以获取结果的子集，针对返回的子集进行渲染优化。</li>
<li><a href="https://redux-toolkit.js.org/rtk-query/api/createApi#refetchonmountorargchange" target="_blank" rel="noopener noreferrer">refetchOnMountOrArgChange<ExternalLinkIcon/></a> - 允许强制查询始终在挂载时重新取回迁（何时提供）。允许在自上次查询同一缓存（当设置为<code v-pre>true</code>）以来已经过去了足够的时间（以秒为单位）时强制查询重新获取。默认为<code v-pre>true</code> <code v-pre>number</code> <code v-pre>false</code></li>
<li><a href="https://redux-toolkit.js.org/rtk-query/api/createApi#refetchonfocus" target="_blank" rel="noopener noreferrer">refetchOnFocus<ExternalLinkIcon/></a>  - 允许在浏览器窗口重新获得焦点时强制查询重新获取。默认为<code v-pre>false</code></li>
<li><a href="https://redux-toolkit.js.org/rtk-query/api/createApi#refetchonreconnect" target="_blank" rel="noopener noreferrer">refetchOnReconnect<ExternalLinkIcon/></a>  - 允许在重新获得网络连接时强制查询重新获取。默认为<code v-pre>false</code></li>
</ul>
<h3 id="_8-1-条件提取" tabindex="-1"><a class="header-anchor" href="#_8-1-条件提取" aria-hidden="true">#</a> 8.1 条件提取</h3>
<p>默认为<code v-pre>false</code>。一旦挂载组件，查询钩子就会自动开始获取数据。但是，在某些用例中，您可能希望延迟获取数据，直到某些条件变为真。RTK 查询支持条件提取以启用该行为。</p>
<p>如果要阻止查询自动运行，可以在钩子中使用参数<code v-pre>skip</code></p>
<p>跳过示例</p>
<div class="language-jsx line-numbers-mode" data-ext="jsx"><pre v-pre class="language-jsx"><code><span class="token keyword">const</span> <span class="token function-variable function">Pokemon</span> <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token parameter"><span class="token punctuation">{</span> name<span class="token punctuation">,</span> skip <span class="token punctuation">}</span></span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
...
<li>如果查询缓存了数据：
<ul>
<li>缓存的数据<strong>将不会在</strong>初始加载时使用，并且将忽略来自任何相同查询的更新，直到删除条件<code v-pre>skip</code></li>
<li>查询的状态为<code v-pre>uninitialized</code></li>
<li>初始加载后设置的 ifis，将使用缓存结果<code v-pre>skip: false</code></li>
</ul>
</li>
<li>如果查询没有缓存数据：
<ul>
<li>查询的状态为<code v-pre>uninitialized</code></li>
<li>使用开发工具查看查询时，查询将不存在于该状态</li>
<li>查询不会在装载时自动获取</li>
<li>当添加具有相同查询的其他组件时，查询不会自动运行</li>
</ul>
</li>
</ul>
<p>这里我想演示的例子是如果我们给钩子函数传递的参数是一个<code v-pre>undefined</code>，这个时候发起请求是会报错的，我们可以使用<code v-pre>skip</code>来来跳过这次无法进行的请求。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里如果父组件传过来的<code v-pre>stuId</code>是个<code v-pre>undefined</code>,那么这次就不会发起请求了。</p>
<h3 id="_8-2-轮询" tabindex="-1"><a class="header-anchor" href="#_8-2-轮询" aria-hidden="true">#</a> 8.2 轮询</h3>
<p>默认值为<code v-pre>0</code>。轮询使您能够通过使查询按指定的时间间隔运行来产生“实时”效果。若要为查询启用轮询，请以毫秒为单位的间隔将值传递给钩子</p>
<div class="language-jsx line-numbers-mode" data-ext="jsx"><pre v-pre class="language-jsx"><code><span class="token keyword">import</span> React <span class="token keyword">from</span> <span class="token string">'react'</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_8-3-从查询结果中选择数据" tabindex="-1"><a class="header-anchor" href="#_8-3-从查询结果中选择数据" aria-hidden="true">#</a> 8.3 从查询结果中选择数据</h3>
<p><code v-pre>selectFromResult</code>允许您以高性能方式从查询结果中获取特定段。使用此功能时，除非所选项的基础数据已更改，否则组件不会重新呈现。如果所选项是较大集合中的一个元素，它将忽略对同一集合中元素的更改。</p>
<p><code v-pre>AllStudent.jsx</code></p>
<div class="language-jsx line-numbers-mode" data-ext="jsx"><pre v-pre class="language-jsx"><code><span class="token keyword">import</span> React <span class="token keyword">from</span> <span class="token string">'react'</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> useGetStudentsQuery <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'./store/features/api/sudentApiSlice'</span>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token keyword">function</span> <span class="token function">App</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token keyword">return</span> <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>div</span><span class="token punctuation">></span></span><span class="token punctuation">{</span>content<span class="token punctuation">}</span><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>div</span><span class="token punctuation">></span></span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>先看这个<code v-pre>selectFromResult</code>方法的参数是什么</p>
<p><img src="https://i0.hdslb.com/bfs/album/0b94738405c63d074254fc70485f6449e4f9f222.png" alt="image-20221105195834361"></p>
<p>这里我们可以对学生数据进行过滤</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token function-variable function">selectFromResult</span><span class="token operator">:</span> <span class="token parameter">result</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
  ...
  <span class="token keyword">return</span> result
<span class="token punctuation">}</span><span class="token punctuation">,</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="https://i0.hdslb.com/bfs/album/1dd255644e0bc2800e6fd13a4a5440fe5818dc38.png" alt="image-20221105201840326"></p>
<h3 id="_8-4-refetchonmountorargchange" tabindex="-1"><a class="header-anchor" href="#_8-4-refetchonmountorargchange" aria-hidden="true">#</a> 8.4 refetchOnMountOrArgChange</h3>
<p>默认为<code v-pre>false</code>。此设置允许您控制缓存结果是否已经可用 RTK 查询将仅提供缓存的结果，或者是否应该设置为 或 自上次成功查询结果以来已经过去了足够的时间。</p>
<ul>
<li><code v-pre>false</code>- 除非查询尚不存在*，否则*不会导致执行查询。</li>
<li><code v-pre>true</code>- 在添加查询的新订阅者时，将始终重新获取。行为与调用回调或传入操作创建者相同。</li>
<li><code v-pre>number</code> - <strong>值以秒为单位</strong>。如果提供了一个数字，并且缓存中存在现有查询，它将比较当前时间与上次实现的时间戳，并且仅在经过足够时间时才重新获取。</li>
</ul>
<p>如果同时指定此选项<code v-pre>skip: true</code>，则在 false 之前<strong>不会对其进行计算</strong>。</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code>  <span class="token keyword">const</span> <span class="token punctuation">{</span>
    <span class="token literal-property property">data</span><span class="token operator">:</span> studentsRes<span class="token punctuation">,</span>
    isLoading<span class="token punctuation">,</span>
    isSuccess<span class="token punctuation">,</span>
    isError<span class="token punctuation">,</span>
...
<p>注意</p>
<p><a href="https://redux-toolkit.js.org/rtk-query/api/createApi#refetchonmountorargchange" target="_blank" rel="noopener noreferrer">fetchBaseQuery |Redux Toolkit (redux-toolkit.js.org)<ExternalLinkIcon/></a></p>
<p>您可以在<code v-pre>createApi</code>中全局设置此项<code v-pre>refetchOnMountOrArgChange</code>，但您也可以覆盖默认值，并通过传递给每个单独的钩子调用或类似地通过 passingwhen 调度<a href="https://redux-toolkit.js.org/rtk-query/api/created-api/endpoints#initiate" target="_blank" rel="noopener noreferrer"><code v-pre>启动</code><ExternalLinkIcon/></a>操作来获得更精细的控制。<code v-pre>createApi</code></p>
</blockquote>
<h3 id="_8-5-refetchonfocus" tabindex="-1"><a class="header-anchor" href="#_8-5-refetchonfocus" aria-hidden="true">#</a> 8.5 refetchOnFocus</h3>
<p>默认值为<code v-pre>false</code>。此设置允许您控制 RTK 查询是否在应用程序窗口重新获得焦点后尝试重新获取所有订阅的查询。</p>
<p>如果同时指定此选项<code v-pre>skip: true</code>，则在 false 之前<strong>不会对其进行计算</strong>。</p>
<p>注意：要求已调用<a href="https://redux-toolkit.js.org/rtk-query/api/setupListeners" target="_blank" rel="noopener noreferrer"><code v-pre>安装程序侦听器</code><ExternalLinkIcon/></a>。</p>
<blockquote>
<p>注意</p>
<p><a href="https://redux-toolkit.js.org/rtk-query/api/createApi#refetchonfocus" target="_blank" rel="noopener noreferrer">fetchBaseQuery |Redux Toolkit (redux-toolkit.js.org)<ExternalLinkIcon/></a></p>
<p>您可以在<code v-pre>createApi</code>中全局设置中此项<code v-pre>refetchOnFocus</code>，但也可以覆盖默认值，并通过传递给每个单独的钩子调用或在调度<a href="https://redux-toolkit.js.org/rtk-query/api/created-api/endpoints#initiate" target="_blank" rel="noopener noreferrer"><code v-pre>启动</code><ExternalLinkIcon/></a>操作时进行更精细的控制。</p>
<p>如果您指定手动分派查询的时间，RTK 查询将无法自动为您重新获取。</p>
</blockquote>
<p>想使用还得为<code v-pre>store</code>添加一个配置才行</p>
<p><code v-pre>store/index.js</code></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> configureStore <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'@reduxjs/toolkit'</span>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> store
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>然后我们看下效果</p>
<p><img src="https://i0.hdslb.com/bfs/album/b1516ee2e8d603cde783a51a7b1438bdf6735714.png" alt="image-20221105203424540"></p>
<p>从<code v-pre>devtool</code>回来点一下网页会重新发一次请求，然后从别的网站点回来也会重新发起请求。</p>
<h3 id="_8-6-refetchonreconnect" tabindex="-1"><a class="header-anchor" href="#_8-6-refetchonreconnect" aria-hidden="true">#</a> 8.6 refetchOnReconnect</h3>
<p>默认值为<code v-pre>false</code>，此设置允许您控制 RTK 查询在重新获得网络连接后是否尝试重新获取所有订阅的查询。</p>
<p>如果同时指定此选项<code v-pre>skip: true</code>，则在 false 之前<strong>不会对其进行计算</strong>。</p>
<p>注意：要求已调用<a href="https://redux-toolkit.js.org/rtk-query/api/setupListeners" target="_blank" rel="noopener noreferrer"><code v-pre>安装程序侦听器</code><ExternalLinkIcon/></a>。</p>
<blockquote>
<p>注意</p>
<p>您可以在<code v-pre>createApi</code>中全局设置此项<code v-pre>refetchOnReconnect</code>，但也可以覆盖默认值，并通过传递给每个单独的钩子调用或在调度<a href="https://redux-toolkit.js.org/rtk-query/api/created-api/endpoints#initiate" target="_blank" rel="noopener noreferrer"><code v-pre>启动</code><ExternalLinkIcon/></a>操作时进行更精细的控制。</p>
<p>如果您指定手动分派查询的时间，RTK 查询将无法自动为您重新获取。<code v-pre>track: false</code></p>
</blockquote>
<h2 id="_9-刷新缓存数据" tabindex="-1"><a class="header-anchor" href="#_9-刷新缓存数据" aria-hidden="true">#</a> 9.刷新缓存数据</h2>
<p>当我们点击<code v-pre>添加学生</code>时，我们可以在浏览器 DevTools 中查看 Network 选项卡，确认 HTTP <code v-pre>POST</code> 请求成功。 但是，如果我们回到<code v-pre>所有学生组件</code>，新的学生信息并不会被展示出来。我们在内存中仍然有相同的缓存数据。</p>
<p>我们需要告诉 RTK Query 刷新其缓存的学生列表，以便我们可以看到我们刚刚添加的新学生信息。</p>
<h3 id="_9-1-手动刷新" tabindex="-1"><a class="header-anchor" href="#_9-1-手动刷新" aria-hidden="true">#</a> 9.1 手动刷新</h3>
<p>第一个选项是手动强制 RTK Query 重新获取给定请求接口的数据。Query hooks 结果对象包含一个 “refetch” 函数，我们可以调用它来强制重新获取。 我们可以暂时将“重新获取学生列表”按钮添加到<code v-pre>&lt;AllStudent&gt;</code>，并在添加新学生后单击该按钮。</p>
<p><code v-pre>AllStudent.jsx</code></p>
<div class="language-jsx line-numbers-mode" data-ext="jsx"><pre v-pre class="language-jsx"><code><span class="token keyword">import</span> React <span class="token keyword">from</span> <span class="token string">'react'</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> useGetStudentsQuery <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'./store/features/api/sudentApiSlice'</span>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token keyword">function</span> <span class="token function">App</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">)</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>首先先从首页添加一个学生数据,然后回到<code v-pre>所有学生组件</code></p>
<p><img src="https://i0.hdslb.com/bfs/album/60d83adac6a06c893f4642031d0af0256e755a2b.png" alt="image-20221106161045089"></p>
<p>这个时候由于有缓存，用的还是之前的数据，我们使用<code v-pre>reFetch</code>方法来强制刷新数据</p>
<p><img src="https://i0.hdslb.com/bfs/album/867b54faf816fb065d0b2a6393652e9f5c49952f.png" alt="image-20221106161736244"></p>
<h3 id="_9-2-缓存失效自动刷新-数据标签" tabindex="-1"><a class="header-anchor" href="#_9-2-缓存失效自动刷新-数据标签" aria-hidden="true">#</a> 9.2 缓存失效自动刷新-数据标签</h3>
<p>有时需要让用户手动单击以重新获取数据，但对于正常使用而言绝对不是一个好的解决方案。</p>
<p>我们知道我们的服务器拥有所有帖子的完整列表，包括我们刚刚添加的帖子。 理想情况下，我们希望我们的应用程序在 Mutation 请求完成后自动重新获取更新的帖子列表。 这样我们就知道我们的客户端缓存数据与服务器所拥有的数据是同步的。</p>
<p><strong>RTK Query 让我们定义查询和 mutations 之间的关系，以启用自动数据重新获取，使用标签</strong>。标签是一个字符串或小对象，可让你命名某些类型的数据和缓存的 <em>无效</em> 部分。当缓存标签失效时，RTK Query 将自动重新获取标记有该标签的请求接口。</p>
<p>基本标签使用需要向我们的 API slice 添加三条信息：</p>
<ul>
<li>API slice 对象中的根 <code v-pre>tagTypes</code> 字段，声明数据类型的字符串标签名称数组，例如 <code v-pre>'student'</code></li>
<li>查询请求接口中的 “providesTags” 数组，列出了一组描述该查询中数据的标签</li>
<li>Mutation 请求接口中的“invalidatesTags”数组，列出了每次 Mutation 运行时失效的一组标签</li>
</ul>
<p>我们可以在 API slice 中添加一个名为 <code v-pre>'student'</code> 的标签，让我们在添加新帖子时自动重新获取 <code v-pre>getStudents</code> 请求接口：</p>
<p><code v-pre>features/api/sudentApiSlice.js</code></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> createApi<span class="token punctuation">,</span> fetchBaseQuery <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'@reduxjs/toolkit/dist/query/react'</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这就是我们所需要的！ 现在，如果我们单击<code v-pre>添加学生</code>，然后回到<code v-pre>AllStudent</code>组件重新发起请求，渲染新的数据</p>
<p>请注意，这里的文字字符串 <code v-pre>'student'</code> 没有什么特别之处。 我们可以称它为“Fred”、“qwerty”或其他任何名称。 它只需要在每个字段中使用相同的字符串，以便 RTK Query 知道“当发生这种 Mutation 时，使列出相同标签字符串的所有请求接口无效”。</p>
</div></template>
