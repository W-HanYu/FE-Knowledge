<template><div><h1 id="_10-【异步组件-组合式函数-hooks-】" tabindex="-1"><a class="header-anchor" href="#_10-【异步组件-组合式函数-hooks-】" aria-hidden="true">#</a> 10 【异步组件 组合式函数(hooks)】</h1>
<h2 id="_1-异步组件" tabindex="-1"><a class="header-anchor" href="#_1-异步组件" aria-hidden="true">#</a> 1.异步组件</h2>
<h3 id="_1-1-为什么要有异步组件" tabindex="-1"><a class="header-anchor" href="#_1-1-为什么要有异步组件" aria-hidden="true">#</a> 1.1 为什么要有异步组件</h3>
<p>首先来看这样一段代码</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="https://i0.hdslb.com/bfs/album/4996cd02d6218ffb28b50b2ec2840146e1e3db4b.png" alt="image-20220809174007084"></p>
<p>把网络切换到慢一点的速度，你会发现这两个组件时同时出来的，所以父组件是会要等子组件加载成功的。如果<code v-pre>Child</code>组件一直没加载成功，那么父组件会一直等着他。</p>
<p>所以引出了异步组件，他可以无需让父组件等待子组件。</p>
<h3 id="_1-2-基础用法" tabindex="-1"><a class="header-anchor" href="#_1-2-基础用法" aria-hidden="true">#</a> 1.2 基础用法</h3>
<p>在大型项目中，我们可能需要拆分应用为更小的块，并仅在需要时再从服务器加载相关组件。Vue 提供了 <a href="https://staging-cn.vuejs.org/api/general.html#defineasynccomponent" target="_blank" rel="noopener noreferrer"><code v-pre>defineAsyncComponent</code><ExternalLinkIcon/></a> 方法来实现此功能</p>
<p><code v-pre>defineAsyncComponent</code> 方法接收一个返回 Promise 的加载函数。这个 Promise 的 <code v-pre>resolve</code> 回调方法应该在从服务器获得组件定义时调用。你也可以调用 <code v-pre>reject(reason)</code> 表明加载失败。</p>
<p><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import#dynamic_imports" target="_blank" rel="noopener noreferrer">ES 模块动态导入<ExternalLinkIcon/></a>也会返回一个 Promise，所以多数情况下我们会将它和 <code v-pre>defineAsyncComponent</code> 搭配使用。类似 Vite 和 Webpack 这样的构建工具也支持此语法（并且会将它们作为打包时的代码分割点），因此我们也可以用它来导入 Vue 单文件组件：</p>
<p>父组件<code v-pre>App.vue</code></p>
<div class="language-vue line-numbers-mode" data-ext="vue"><pre v-pre class="language-vue"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>template</span><span class="token punctuation">></span></span>
  <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>div</span> <span class="token attr-name">class</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>app<span class="token punctuation">"</span></span><span class="token punctuation">></span></span>
    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>h3</span><span class="token punctuation">></span></span>我是App组件<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>h3</span><span class="token punctuation">></span></span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>script</span> <span class="token attr-name">setup</span><span class="token punctuation">></span></span><span class="token script"><span class="token language-javascript">
<span class="token comment">// import Child from './components/Child.vue';</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="https://i0.hdslb.com/bfs/album/5b63169ca48efebcd67ddb9036c7a504ac3de64d.png" alt="image-20220809174530192"></p>
<p>这个时候父组件已经能不用等子组件了，但是，这样还有一个问题：</p>
<p>子组件是占用了父组件的空间的，闪一下突然加载出来很不友好，所以引出了内置组件<code v-pre>Suspense </code>。</p>
<h3 id="_1-3-加载与错误状态" tabindex="-1"><a class="header-anchor" href="#_1-3-加载与错误状态" aria-hidden="true">#</a> 1.3 加载与错误状态</h3>
<p>异步操作不可避免地会涉及到加载和错误状态，因此 <code v-pre>defineAsyncComponent()</code> 也支持在高级选项中处理这些状态：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> AsyncComp <span class="token operator">=</span> <span class="token function">defineAsyncComponent</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
...
<h3 id="_1-4-搭配-suspense-使用-实验性" tabindex="-1"><a class="header-anchor" href="#_1-4-搭配-suspense-使用-实验性" aria-hidden="true">#</a> 1.4 搭配 Suspense 使用（实验性）</h3>
<p>异步组件可以搭配内置的 <code v-pre>&lt;Suspense&gt;</code> 组件一起使用，若想了解 <code v-pre>&lt;Suspense&gt;</code> 和异步组件之间交互，请参阅 <a href="https://staging-cn.vuejs.org/guide/built-ins/suspense.html" target="_blank" rel="noopener noreferrer">``<ExternalLinkIcon/></a> 章节。</p>
<p><code v-pre>&lt;Suspense&gt;</code> 是一个内置组件，用来在组件树中协调对异步依赖的处理。它让我们可以在组件树上层等待下层的多个嵌套异步依赖项解析完成，并可以在等待时渲染一个加载状态。</p>
<h4 id="_1-4-1-异步依赖" tabindex="-1"><a class="header-anchor" href="#_1-4-1-异步依赖" aria-hidden="true">#</a> 1.4.1 异步依赖</h4>
<p>要了解 <code v-pre>&lt;Suspense&gt;</code> 所解决的问题和它是如何与异步依赖进行交互的，我们需要想象这样一种组件层级结构：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>&lt;Suspense>
└─ &lt;Dashboard>
   ├─ &lt;Profile>
   ...
   └─ &lt;Content>
      ├─ &lt;ActivityFeed> （异步组件）
      └─ &lt;Stats>（异步组件）
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这个组件树中有多个嵌套组件，要渲染出它们，首先得解析一些异步资源。如果没有 <code v-pre>&lt;Suspense&gt;</code>，则它们每个都需要处理自己的加载、报错和完成状态。在最坏的情况下，我们可能会在页面上看到三个旋转的加载态，在不同的时间显示出内容。</p>
<p>有了 <code v-pre>&lt;Suspense&gt;</code> 组件后，我们就可以在等待整个多层级组件树中的各个异步依赖获取结果时，在顶层展示出加载中或加载失败的状态。</p>
<p><code v-pre>&lt;Suspense&gt;</code> 可以等待的异步依赖有两种：</p>
<ol>
<li>带有异步 <code v-pre>setup()</code> 钩子的组件。这也包含了使用 <code v-pre>&lt;script setup&gt;</code> 时有顶层 <code v-pre>await</code> 表达式的组件。</li>
<li><a href="https://staging-cn.vuejs.org/guide/components/async.html" target="_blank" rel="noopener noreferrer">异步组件<ExternalLinkIcon/></a>。</li>
</ol>
<p><code v-pre>async setup()</code><a href="https://staging-cn.vuejs.org/guide/built-ins/suspense.html#async-setup" target="_blank" rel="noopener noreferrer">#<ExternalLinkIcon/></a></p>
<p>组合式 API 中组件的 <code v-pre>setup()</code> 钩子可以是异步的：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果使用 <code v-pre>&lt;script setup&gt;</code>，那么顶层 <code v-pre>await</code> 表达式会自动让该组件成为一个异步依赖：</p>
...
<span class="token operator">&lt;</span>template<span class="token operator">></span>
  <span class="token punctuation">{</span><span class="token punctuation">{</span> posts <span class="token punctuation">}</span><span class="token punctuation">}</span>
...
<p>异步组件默认就是 <strong>“suspensible”</strong> 的。这意味着如果组件关系链上有一个 <code v-pre>&lt;Suspense&gt;</code>，那么这个异步组件就会被当作这个 <code v-pre>&lt;Suspense&gt;</code> 的一个异步依赖。在这种情况下，加载状态是由 <code v-pre>&lt;Suspense&gt;</code> 控制，而该组件自己的加载、报错、延时和超时等选项都将被忽略。</p>
<p>异步组件也可以通过在选项中指定 <code v-pre>suspensible: false</code> 表明不用 <code v-pre>Suspense</code> 控制，并让组件始终自己控制其加载状态。</p>
</blockquote>
<h4 id="_1-4-2-加载中状态" tabindex="-1"><a class="header-anchor" href="#_1-4-2-加载中状态" aria-hidden="true">#</a> 1.4.2 加载中状态</h4>
<p><code v-pre>&lt;Suspense&gt;</code> 组件有两个插槽：<code v-pre>#default</code> 和 <code v-pre>#fallback</code>。两个插槽都只允许<strong>一个</strong>直接子节点。在可能的时候都将显示默认槽中的节点。否则将显示后备槽中的节点。</p>
<div class="language-vue line-numbers-mode" data-ext="vue"><pre v-pre class="language-vue"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>Suspense</span><span class="token punctuation">></span></span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在初始渲染时，<code v-pre>&lt;Suspense&gt;</code> 将在内存中渲染其默认的插槽内容。如果在这个过程中遇到任何异步依赖，则会进入<strong>挂起</strong>状态。在挂起状态期间，展示的是后备内容。当所有遇到的异步依赖都完成后，<code v-pre>&lt;Suspense&gt;</code> 会进入<strong>完成</strong>状态，并将展示出默认插槽的内容。</p>
<p>如果在初次渲染时没有遇到异步依赖，<code v-pre>&lt;Suspense&gt;</code> 会直接进入完成状态。</p>
<p>进入完成状态后，只有当默认插槽的根节点被替换时，<code v-pre>&lt;Suspense&gt;</code> 才会回到挂起状态。组件树中新的更深层次的异步依赖<strong>不会</strong>造成 <code v-pre>&lt;Suspense&gt;</code> 回退到挂起状态。</p>
<p>发生回退时，后备内容不会立即展示出来。相反，<code v-pre>&lt;Suspense&gt;</code> 在等待新内容和异步依赖完成时，会展示之前 <code v-pre>#default</code> 插槽的内容。这个行为可以通过一个 <code v-pre>timeout</code> prop 进行配置：在等待渲染新内容耗时超过 <code v-pre>timeout</code> 之后，<code v-pre>&lt;Suspense&gt;</code> 将会切换为展示后备内容。若 <code v-pre>timeout</code> 值为 <code v-pre>0</code> 将导致在替换默认内容时立即显示后备内容。</p>
<h4 id="_1-4-3-解决上面遗留的问题" tabindex="-1"><a class="header-anchor" href="#_1-4-3-解决上面遗留的问题" aria-hidden="true">#</a> 1.4.3 解决上面遗留的问题</h4>
<p>父组件<code v-pre>App.vue</code></p>
<div class="language-vue line-numbers-mode" data-ext="vue"><pre v-pre class="language-vue"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>template</span><span class="token punctuation">></span></span>
  <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>div</span> <span class="token attr-name">class</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>app<span class="token punctuation">"</span></span><span class="token punctuation">></span></span>
    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>h3</span><span class="token punctuation">></span></span>我是App组件<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>h3</span><span class="token punctuation">></span></span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>script</span> <span class="token attr-name">setup</span><span class="token punctuation">></span></span><span class="token script"><span class="token language-javascript">
<span class="token keyword">let</span> sum <span class="token operator">=</span> <span class="token function">$ref</span><span class="token punctuation">(</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>加载时</strong></p>
<p><img src="https://i0.hdslb.com/bfs/album/081847b37f38a3c17aa89b300364961df05bad7c.png" alt="image-20220809180418781"></p>
<p><strong>加载完成</strong></p>
<p><img src="https://i0.hdslb.com/bfs/album/94478819d69f20e253caa6437c11d377ac669ad0.png" alt="image-20220809180430412"></p>
<h2 id="_2-组合式函数-hooks" tabindex="-1"><a class="header-anchor" href="#_2-组合式函数-hooks" aria-hidden="true">#</a> 2.组合式函数(hooks)</h2>
<h3 id="_2-1-什么是-组合式函数" tabindex="-1"><a class="header-anchor" href="#_2-1-什么是-组合式函数" aria-hidden="true">#</a> 2.1 什么是“组合式函数”？</h3>
<p>在 Vue 应用的概念中，“组合式函数” (Composables) 是一个利用 Vue 组合式 API 来封装和复用<strong>有状态逻辑</strong>的函数。</p>
<p>当构建前端应用时，我们常常需要复用公共任务的逻辑。例如为了在不同地方格式化时间，我们可能会抽取一个可复用的日期格式化函数。这个函数封装了<strong>无状态的逻辑</strong>：它在接收一些输入后立刻返回所期望的输出。复用无状态逻辑的库有很多，比如你可能已经用过的 <a href="https://lodash.com/" target="_blank" rel="noopener noreferrer">lodash<ExternalLinkIcon/></a> 或是 <a href="https://date-fns.org/" target="_blank" rel="noopener noreferrer">date-fns<ExternalLinkIcon/></a>。</p>
<p>相比之下，有状态逻辑负责管理会随时间而变化的状态。一个简单的例子是跟踪当前鼠标在页面中的位置。在实际应用中，也可能是像触摸手势或与数据库的连接状态这样的更复杂的逻辑。</p>
<h3 id="_2-2-鼠标跟踪器示例" tabindex="-1"><a class="header-anchor" href="#_2-2-鼠标跟踪器示例" aria-hidden="true">#</a> 2.2 鼠标跟踪器示例</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="https://i0.hdslb.com/bfs/album/39f70b7b2e52cb51ceb6f53716458658214e2521.png" alt="image-20220809190244670"></p>
<p>但是，如果我们想在多个组件中复用这个相同的逻辑呢？我们可以把这个逻辑以一个组合式函数的形式提取到外部文件中：</p>
<p><code v-pre>hooks/mouse.js</code></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// mouse.js</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> ref<span class="token punctuation">,</span> onMounted<span class="token punctuation">,</span> onUnmounted <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'vue'</span>
...
<div class="language-vue line-numbers-mode" data-ext="vue"><pre v-pre class="language-vue"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>script</span> <span class="token attr-name">setup</span><span class="token punctuation">></span></span><span class="token script"><span class="token language-javascript">
<span class="token keyword">import</span> <span class="token punctuation">{</span> useMouse <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'@/hooks/mouse.js'</span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>template</span><span class="token punctuation">></span></span>Mouse position is at: {{ x }}, {{ y }}<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>template</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><a href="https://sfc.vuejs.org/#" target="_blank" rel="noopener noreferrer">在演练场中尝试一下<ExternalLinkIcon/></a></p>
<p>如你所见，核心逻辑完全没变，我们做的只是把它移到一个外部函数中去，并返回需要暴露的状态。和在组件中一样，你也可以在组合式函数中使用所有的<a href="https://staging-cn.vuejs.org/api/#composition-api" target="_blank" rel="noopener noreferrer">组合式 API<ExternalLinkIcon/></a>。现在，<code v-pre>useMouse()</code> 的功能可以在任何组件中轻易复用了。</p>
...
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// hooks/event.js</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>有了它，之前的 <code v-pre>useMouse()</code> 可以被简化为：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// hooks/mouse.js</span>
...
<span class="token keyword">export</span> <span class="token keyword">function</span> <span class="token function">useMouse</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token function">useEventListener</span><span class="token punctuation">(</span>window<span class="token punctuation">,</span> <span class="token string">'mousemove'</span><span class="token punctuation">,</span> <span class="token parameter">event</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
    x<span class="token punctuation">.</span>value <span class="token operator">=</span> event<span class="token punctuation">.</span>pageX
    y<span class="token punctuation">.</span>value <span class="token operator">=</span> event<span class="token punctuation">.</span>pageY
  <span class="token punctuation">}</span><span class="token punctuation">)</span>
...
<p>每一个调用 <code v-pre>useMouse()</code> 的组件实例会创建其独有的 <code v-pre>x</code>、<code v-pre>y</code> 状态拷贝，因此他们不会互相影响。如果你想要在组件之间共享状态，请阅读<a href="https://staging-cn.vuejs.org/guide/scaling-up/state-management.html" target="_blank" rel="noopener noreferrer">状态管理<ExternalLinkIcon/></a>这一章。</p>
</blockquote>
<h3 id="_2-3-异步状态示例" tabindex="-1"><a class="header-anchor" href="#_2-3-异步状态示例" aria-hidden="true">#</a> 2.3 异步状态示例</h3>
<p>以后用到再补充</p>
<h3 id="_2-4-约定和最佳实践" tabindex="-1"><a class="header-anchor" href="#_2-4-约定和最佳实践" aria-hidden="true">#</a> 2.4 约定和最佳实践</h3>
<h4 id="_2-4-1-命名" tabindex="-1"><a class="header-anchor" href="#_2-4-1-命名" aria-hidden="true">#</a> 2.4.1 命名<a href="https://staging-cn.vuejs.org/guide/reusability/composables.html#naming" target="_blank" rel="noopener noreferrer">#<ExternalLinkIcon/></a></h4>
<p>组合式函数约定用驼峰命名法命名，并以“use”作为开头。</p>
<h4 id="_2-4-2-输入参数" tabindex="-1"><a class="header-anchor" href="#_2-4-2-输入参数" aria-hidden="true">#</a> 2.4.2 输入参数<a href="https://staging-cn.vuejs.org/guide/reusability/composables.html#input-arguments" target="_blank" rel="noopener noreferrer">#<ExternalLinkIcon/></a></h4>
<p>尽管其响应性不依赖 ref，组合式函数仍可接收 ref 参数。如果编写的组合式函数会被其他开发者使用，你最好在处理输入参数时兼容 ref 而不只是原始的值。<a href="https://staging-cn.vuejs.org/api/reactivity-utilities.html#unref" target="_blank" rel="noopener noreferrer"><code v-pre>unref()</code><ExternalLinkIcon/></a> 工具函数会对此非常有帮助：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> unref <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'vue'</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果你的组合式函数在接收 ref 为参数时会产生响应式 effect，请确保使用 <code v-pre>watch()</code> 显式地监听此 ref，或者在 <code v-pre>watchEffect()</code> 中调用 <code v-pre>unref()</code> 来进行正确的追踪。</p>
<h4 id="_2-4-3-返回值" tabindex="-1"><a class="header-anchor" href="#_2-4-3-返回值" aria-hidden="true">#</a> 2.4.3 返回值<a href="https://staging-cn.vuejs.org/guide/reusability/composables.html#return-values" target="_blank" rel="noopener noreferrer">#<ExternalLinkIcon/></a></h4>
<p>你可能已经注意到了，我们一直在组合式函数中使用 <code v-pre>ref()</code> 而不是 <code v-pre>reactive()</code>。我们推荐的约定是组合式函数始终返回一个包含多个 ref 的普通的非响应式对象，这样该对象在组件中被解构为 ref 之后仍可以保持响应性：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// x 和 y 是两个 ref</span>
<span class="token keyword">const</span> <span class="token punctuation">{</span> x<span class="token punctuation">,</span> y <span class="token punctuation">}</span> <span class="token operator">=</span> <span class="token function">useMouse</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>从组合式函数返回一个响应式对象会导致在对象解构过程中丢失与组合式函数内状态的响应性连接。与之相反，ref 则可以维持这一响应性连接。</p>
<p>如果你更希望以对象属性的形式来使用组合式函数中返回的状态，你可以将返回的对象用 <code v-pre>reactive()</code> 包装一次，这样其中的 ref 会被自动解包，例如：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> mouse <span class="token operator">=</span> <span class="token function">reactive</span><span class="token punctuation">(</span><span class="token function">useMouse</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2-4-4-副作用" tabindex="-1"><a class="header-anchor" href="#_2-4-4-副作用" aria-hidden="true">#</a> 2.4.4 副作用<a href="https://staging-cn.vuejs.org/guide/reusability/composables.html#side-effects" target="_blank" rel="noopener noreferrer">#<ExternalLinkIcon/></a></h4>
<p>在组合式函数中的确可以执行副作用 (例如：添加 DOM 事件监听器或者请求数据)，但请注意以下规则：</p>
<ul>
<li>如果你的应用用到了<a href="https://staging-cn.vuejs.org/guide/scaling-up/ssr.html" target="_blank" rel="noopener noreferrer">服务端渲染<ExternalLinkIcon/></a> (SSR)，请确保在组件挂载后才调用的生命周期钩子中执行 DOM 相关的副作用，例如：<code v-pre>onMounted()</code>。这些钩子仅会在浏览器中被调用，因此可以确保能访问到 DOM。</li>
<li>确保在 <code v-pre>onUnmounted()</code> 时清理副作用。举例来说，如果一个组合式函数设置了一个事件监听器，它就应该在 <code v-pre>onUnmounted()</code> 中被移除 (就像我们在 <code v-pre>useMouse()</code> 示例中看到的一样)。当然也可以像之前的 <code v-pre>useEventListener()</code> 示例那样，使用一个组合式函数来自动帮你做这些事。</li>
</ul>
<h4 id="_2-4-5-使用限制" tabindex="-1"><a class="header-anchor" href="#_2-4-5-使用限制" aria-hidden="true">#</a> 2.4.5 使用限制<a href="https://staging-cn.vuejs.org/guide/reusability/composables.html#usage-restrictions" target="_blank" rel="noopener noreferrer">#<ExternalLinkIcon/></a></h4>
<p>组合式函数在 <code v-pre>&lt;script setup&gt;</code> 或 <code v-pre>setup()</code> 钩子中，应始终被<strong>同步地</strong>调用。在某些场景下，你也可以在像 <code v-pre>onMounted()</code> 这样的生命周期钩子中使用他们。</p>
...
<blockquote>
<p><strong>提示</strong></p>
<p><code v-pre>&lt;script setup&gt;</code>是唯一在调用 await 之后仍可调用组合式函数的地方。编译器会在异步操作之后自动为你恢复当前的组件实例。</p>
</blockquote>
<h3 id="_2-5-通过抽取组合式函数改善代码结构" tabindex="-1"><a class="header-anchor" href="#_2-5-通过抽取组合式函数改善代码结构" aria-hidden="true">#</a> 2.5 通过抽取组合式函数改善代码结构<a href="https://staging-cn.vuejs.org/guide/reusability/composables.html#extracting-composables-for-code-organization" target="_blank" rel="noopener noreferrer">#<ExternalLinkIcon/></a></h3>
